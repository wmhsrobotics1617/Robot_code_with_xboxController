...
//	private String m_autoSelected;
    ...
    //joyLeft  = new Joystick(0);  //move joy stick positions as needed in the driver station.. or change here
...
//			SmartDashboard.putNumber("timer count ", iTimerCount);
    ...
    {
        if(con.getAButton() && !TRIPPED_ELEVATOR_MOTORS)
        {
        ...
        {
            if(con.getBButton() && !TRIPPED_ELEVATOR_MOTORS)
            {
    ...
    public void linkIntakeMotorsToLeftJoyButtons()
    {
        ...
        if(con.getTriggerAxis(GenericHID.Hand.kRight) > .5 && !TRIPPED_INTAKE_MOTORS)
        {
            intakeTargetState = 1;
        }
        else
        {
            if(con.getTriggerAxis(GenericHID.Hand.kLeft) > 0.5&& !TRIPPED_INTAKE_MOTORS)
            {
                intakeTargetState = 2;
            }
            else
    ...
    public void linkDriveMotorsToJoySticks()
    {
        ...
        double leftDriveTarget = con.getY(GenericHID.Hand.kLeft);
        double rightDriveTarget = con.getY(GenericHID.Hand.kRight);
